defmodule RegistryTest do
  ...
  describe ".join_hordes/2" do
    test "two hordes can join each other" do
      {:ok, horde_1} = Horde.Registry.start_link(name: :horde_1)
      {:ok, horde_2} = Horde.Registry.start_link(name: :horde_2)
    ...
    test "three hordes can join in one giant horde" do
      {:ok, horde_1} = Horde.Registry.start_link(name: :horde_1)
      {:ok, horde_2} = Horde.Registry.start_link(name: :horde_2)
      {:ok, horde_3} = Horde.Registry.start_link(name: :horde_3)
      Horde.Cluster.join_hordes(horde_1, horde_2)
    ...
    test "25 hordes can join in one gargantuan horde" do
      last_horde =
        ...
        |> Enum.reduce(nil, fn x, last_horde ->
          {:ok, horde} = Horde.Registry.start_link(name: :"horde_#{x}")
  ...
  describe "register via callbacks" do
    setup do
      {:ok, horde} = Horde.Registry.start_link(name: Horde.Registry.Cluster)
      {:ok, horde: horde}
    end

    test "register a name the 'via' way", %{horde: horde} do
      name = {:via, Horde.Registry, {Horde.Registry.Cluster, "precious"}}
  ...
  describe "lookup" do
    ...
    test "existing process the 'via' way", %{carmen: carmen} do
      name = {:via, Horde.Registry, {Horde.Registry.Cluster, :carmen}}
