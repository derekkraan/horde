defmodule Horde.DynamicSupervisorImpl.Rebalancer do
  ...
  def handle_info({:rebalance, dsi_state, from}, state) do
    cspec_by_pid =
      Map.values(dsi_state.processes_by_id)
    ...
    rebalanced =
      ...
      |> Enum.map(fn pid ->
        ...
        {:ok, %Horde.DynamicSupervisor.Member{name: new_member}} =
          dsi_state.distribution_strategy.choose_node(
            identifier,
            Map.values(dsi_state.members_info)
          )
        ...
        case current_member == new_member do
          false ->
            ...
            Logger.info(
              "Redistributing node: #{Kernel.inspect(pid)} | #{Kernel.inspect(current_member)} -> #{
                Kernel.inspect(new_member)
              } | #{Kernel.inspect(name)}"
            )
            ...
            case Horde.DynamicSupervisor.terminate_child(dsi_state.name, pid) do
              ...
              {:error, :not_found} ->
                Logger.error(
                  "Error redistributing #{Kernel.inspect(pid)} to node #{
                    Kernel.inspect(new_member)
